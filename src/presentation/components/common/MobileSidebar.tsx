...
export const MobileSidebar: React.FC<MobileSidebarProps> = ({ isOpen, onClose }) => {
  ...
  const roleDisplay = role === 'admin'
    ? 'Administrator'
    : (role === 'kepala bagian' || role === 'kasubag')
    ? 'Kepala Bagian'
    : 'Tenaga Ahli';
    ...
    >
      ...
      >
        ...
        <nav className="p-3 flex flex-col gap-1 flex-1 overflow-y-auto overscroll-contain">
          ...
          {isAdmin && (
            <NavItem index={1} href="/admin/dashboard" icon="fa-chart-pie" label={(role === 'kepala bagian' || role === 'kasubag') ? 'Dashboard Kepala Bagian' : 'Admin Dashboard'} />
          )}
